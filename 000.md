...
append-only logs contain the full JSON-encoding of the respective SSB
data structure. The choice of JSON, which has no support for binary
data, mandates that byte arrays are specially encoded (as ASCII
strings according to BASE64), which further bloats a log entry. By
choosing a binary representation, considerable space savings can be
achieved and unnecessary transformations of the representation can be
avoided. CBOR (see References section) was not considered because of
the possibility of indefinite lists which prevent the skipping of
a list without parsing its elements.
